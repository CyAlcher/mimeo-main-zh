...
async def _critique(
    *,
    ...
    system = (
        "你是一名对抗式编辑。你从语气、覆盖、重复、准确性四个维度审阅草稿，"
        "精确具体、绝不泛泛而谈；你的任务是找出真问题，不是安抚作者。"
        "所有 summary、description、suggestion、strengths 必须使用简体中文；"
        "人名、公司、英文书名可以保留原文。"
    )
...
def _render_markdown(
    ...
    severity_zh = {"high": "严重", "medium": "中等", "low": "轻微"}
    lines: list[str] = [
        f"# {title} 评审报告",
        "",
    ...
    if report.issues:
        ...
        for severity in ("high", "medium", "low"):
            ...
            if not items:
                continue
            lines.append(f"### {severity_zh[severity]}")
            lines.append("")
            for issue in items:
                lines.append(f"- **[{issue.category}]** _{issue.location}_")
                lines.append(f"  - {issue.description}")
                if issue.suggestion:
                    lines.append(f"  - _建议：_ {issue.suggestion}")
            lines.append("")
    else:
        lines.append("_本次评审未发现问题。_")
        lines.append("")
    return "\n".join(lines).rstrip() + "\n"
