...
async def _distill_chunk(
    *,
    ...
    if chunk_index is not None and chunk_count is not None and chunk_count > 1:
        body = (
            f"[这是该长文的第 {chunk_index + 1} / {chunk_count} 段。"
            "只根据本段实际内容抽取，不要脑补你没看到的其它段落。]\n\n"
            f"{body}"
        )
    ...
    prompt = render_prompt(
        ...
        expert_context=expert_context,
        source_id=source.id,
        title=fetched.title or source.title or "(无标题)",
        url=source.url,
        kind=source.kind,
        content=body,
    )
    ...
    system = (
        "你是一名严谨的研究分析师。你阅读一手材料，抽取该专家思考过程的"
        "结构化数据。绝不编造；宁可信息缺失，也不要凭空生成。"
        "所有输出字段必须使用简体中文；人名、机构名、英文书名保留原文。"
    )
