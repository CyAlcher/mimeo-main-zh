...
async def _cluster_batch(
    *,
    ...
    system = (
        "你是一名一丝不苟的研究综合师。你要把多份嘈杂的单源抽取合并成"
        "一份干净统一的知识库，保留出处、杜绝臆造。所有输出字段请使用"
        "简体中文；人名、机构、英文书名保留原文。"
    )
...
async def author_skill(
    *,
    ...
    system = (
        "你撰写的 Agent Skill 要让 AI 助手以该专家的风格进行推理。"
        "遵循 skill-creator 规范（YAML frontmatter 的 description 决定触发，"
        "skill_body 控制在 400 行以内，细节放进 references/）。"
        "绝不扮演专家本人；你是借用他的思考方式。每条引文必须一字不差。"
        "所有文本字段与 markdown 均用简体中文输出；人名、机构、英文书名"
        "保留原文。"
    )
...
async def author_agents(
    *,
    ...
    system = (
        "你撰写的 AGENTS.md 会被 AI 编程 Agent 在每次会话开始时读取，"
        "相当于把一位专家的默认思维装进 Agent。它没有 frontmatter、也没有"
        "触发描述，更没有 references/ 拆分——所有要点都必须在一份文件里。"
        "绝不扮演专家本人；你是采纳他的默认立场。每条引文必须一字不差、"
        "带 source id 注明。整份 AGENTS.md 使用简体中文；人名与英文书名"
        "保留原文。"
    )
    output = await llm.structured(
        system=system,
